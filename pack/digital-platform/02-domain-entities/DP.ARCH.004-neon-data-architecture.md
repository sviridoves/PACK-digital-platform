...
**Полный физический инвентарь Neon prod (29 апр 2026, после WP-268 DROP platform):** 12 entity-БД + 4 special-БД = 16 БД (плюс `postgres` = system, не считается).
...
| **4 special (operational support)** | health (observability — internal_metrics, error_logs, ANTHROPIC_STATUS), metabase (admin-tool metadata, paused), payment_registry (autopay tokens, отдельная БД по B7.3 — payment_credentials строже PII), postgres (system) | Не доменные сущности; параллельно entity-уровню (§10.10 admin/state-tools placement) |
...
**Tech debt — что не сделано (по плану в Plan agent 29 апр):**
- **G1 Identity core** (users, ory_tokens, oauth_pending_states, user_integrations) → `persona` (#1) — критический блокер для G2/G3/G6.
- **G2 User state/sessions** (user_state, user_sessions, feed_*) → `journal` (#2) или Railway-local.
- **G3 Billing** (subscription_grants, subscriptions, finance_payments, service_usage) → `subscription` (#4) + `payment` (#3) через payment-registry.
- **G4 Analytics events** (development.user_events) → `journal.event_stream`.
- **G5 Notifications/traces/errors** (notification_log, request_traces, error_logs) → `learning.domain_event` (writer-cutover; read-path уже мигрирован WP-253).
- **G6 Indicators / DT** (digital_twins) → `indicators.calculated_profile` (writer-cutover).
- **G7 Publications** (published_posts, scheduled_publications, channel_*) → `publication` (#9).
- **G8 Learning artefacts** (marathon_content, reminders, training_*, pending_fixes) → `learning` (#6) + `community` (#10).
- **G9 Cache** (content_cache) → Redis или local.

**Когда:** не раньше W19. Параллельные потоки возможны (G5+G6 — независимы от G1; G3 — после G1; G4 — после G1).

**Эстимат:** 76-114h aggregate, 3.5-5 календарных недель при 3 параллельных потоках.
